...
class Command(BaseCommand):
    help = "Tự động chuyển các điều động tạm thời quá hạn từ ACTIVE sang EXPIRED."

    def add_arguments(self, parser):
        parser.add_argument(
            "--date",
            type=str,
            help="Chạy giả lập theo ngày định dạng YYYY-MM-DD.",
        )
        parser.add_argument(
            "--silent",
            action="store_true",
            help="Không in chi tiết.",
        )
    ...
    def handle(self, *args, **options):
        ...
        if not options.get("silent"):
            if stats["expired"] == 0:
                ...
                self.stdout.write(
                    "Đã chuyển EXPIRED. Lưu ý: EXPIRED vẫn có hiệu lực lịch sử trong khoảng ngày điều động."
                )
