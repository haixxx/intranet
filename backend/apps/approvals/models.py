...
class ApprovalFlow(models.Model):
    ...
    scope_unit_id = models.IntegerField(null=True, blank=True, help_text=_("Phạm vi đơn vị áp dụng (tùy chọn)"))
    admin_unit_id = models.IntegerField(null=True, blank=True, help_text=_("Đơn vị quản lý lãnh đạo (ưu tiên ở flow)"))
    settings_json = models.JSONField(default=dict, help_text=_("Cấu hình chung cho luồng (SLA, ủy quyền, chính sách...)"))
    current_version = models.IntegerField(null=True, blank=True)
    created_by = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL, related_name="approval_flows_created")
...
class ApprovalFlowVersion(models.Model):
    ...
    published_by = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL, related_name="approval_flow_versions_published")
...
class ApprovalRequest(models.Model):
    ...
    object_type = models.CharField(max_length=100, help_text=_("Loại đối tượng nguồn (ví dụ: attendance_correction)"))
    ...
    requester = models.ForeignKey(User, on_delete=models.PROTECT, related_name="approval_requests_requested")
    status = models.CharField(max_length=20, choices=Status.choices, default=Status.DRAFT)
...
class ApprovalStep(models.Model):
    class Type(models.TextChoices):
        ...
        DEPT_HEADS_FROM_PREV = "DEPT_HEADS_FROM_PREV", _("Chỉ huy các phòng tham gia")
    ...
    resolver_config = models.JSONField(default=dict, help_text=_("Tham số xác định người ký (JSON)"))
...
class ApprovalSigner(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.PROTECT, related_name="approval_signers")
    role_title = models.CharField(max_length=100, blank=True, default="", help_text=_("Chức danh snapshot (nếu có)"))
    unit_id = models.IntegerField(null=True, blank=True, help_text=_("Đơn vị snapshot (nếu có)"))
    group_key = models.CharField(max_length=100, blank=True, default="", help_text=_("Khóa nhóm (phòng) nếu dùng GROUP_ANY_ALL"))
    is_required = models.BooleanField(default=True)
    source = models.CharField(max_length=20, choices=Source.choices, default=Source.RESOLVED)
    status = models.CharField(max_length=20, choices=Status.choices, default=Status.PENDING)
    acted_at = models.DateTimeField(null=True, blank=True)
    delegated_to = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL, related_name="approval_signers_delegated")
...
class ApprovalAction(models.Model):
    ...
    actor = models.ForeignKey(User, on_delete=models.PROTECT, related_name="approval_actions")
