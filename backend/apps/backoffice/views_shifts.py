...
def shift_list(request):
    items = ShiftTemplate.objects.order_by('code')
...
def shift_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            obj = form.save()
            audit_log(action_verb="CREATE", object_type="shift", object_id=obj.id, object_repr=obj.code,
                      actor=request.user, changes={'fields': {'code': obj.code, 'type': obj.type}}, request=request, action_code="SHIFT_TEMPLATE_CREATE")
...
def shift_edit(request, pk):
    ...
    if request.method == 'POST':
        old = {'name': obj.name, 'type': obj.type, 'start_time': obj.start_time, 'end_time': obj.end_time, 'crosses_midnight': obj.crosses_midnight}
        ...
        if form.is_valid():
            ...
            if diff:
                audit_log(action_verb="UPDATE", object_type="shift", object_id=updated.id, object_repr=updated.code,
                          actor=request.user, changes=diff, request=request, action_code="SHIFT_TEMPLATE_UPDATE")
...
def shift_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        obj.delete()
        audit_log(action_verb="DELETE", object_type="shift", object_id=oid, object_repr=code,
                  actor=request.user, request=request, action_code="SHIFT_TEMPLATE_DELETE")
